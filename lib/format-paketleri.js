...
//   paor_aveva_* — PAOR "BOM Excel'de yaşar" ailesi ($0, AI yok malzeme/kimlik için).
//     Veri: Isometric_View.pdf (kimlik, metin) + *.xlsx (BOM) + ana vektör -A.pdf (çizim).
//     Malzeme + kimlik Excel'den ($0, GÜVENİLİR kaynak). PAOR regex_text AILE_KAYIT'ina
//     BAĞLANMAZ (boş PDF metnine regex→fail→L3 tuzağı). Excel kanalı: lib/paor.js
//     (partNameParse/kimlikCoz/convert). pipeline-malzeme=paor.js→pipeline_malzemeleri.
//   ⚠ 182/MK-182 GÜNCELLEME (MK-180'in 2 kısmı AŞILDI — bkz MK-181.6 + MK-182):
//     · Ana -A.pdf ARTIK L3 ile OKUNUR (spool-bölme). MK-180 "L3'e gitmez" KÖR maliyet
//       kuralıydı (L3 toggle yokken); artık toggle ile SEÇİCİ kontrol var.
//     · Spool-bölme L3'ten gelir (insan-onaylı, manuel_onay/4-durum) — "sessizce PDF'ten
//       spool yaratma" ilkesi korunur. Eşleştirme: dosyaAdiParse DOSYA_DESENLERI (pipeline
//       dosya adından, Tersan kuralı; spool parse'tan + [n]→S0n). Detay: docs/KARARLAR.md MK-182.
//
