  ...
  parser_kural_parcasi: {
    ...
    malzeme_tablosu: {
      ...
      satir_tipleri_ekle: [
        ...
        {
          // + 165 (tersan.zip kaniti, 3 gemi/10 satir): 'Bilezik Detay A' varyanti —
          // L= TASIMAZ, DN sonrasi tek sayi = genislik. Iki sessizlik sinifi birden:
          // 'Ic Bilezik Detay' eski bilezik pattern'ine takilip HAM dusuyordu (7 satir),
          // 'Dis Bilezik Detay' tetik 'Ic Bilezik'e hic uymayip SESSIZCE dusuyordu
          // (3 satir — MK-123.C manson sinifi). Tetik 'Bilezik Detay' ikisini de kapsar
          // (Turkce I/s glyph riskinden kacinmak icin on-ek tetikte yok). spesifiklik 6:
          // klasik L='li bilezikten (5) ONCE denenir; L='li satirlar bu tetige uymaz.
          ad: 'bilezik_detay', kategori: 'fitting', spesifiklik: 6,
          tetikleyici_regex: 'Bilezik Detay',
          pattern: '^\\d+(\\d)(.+?)DN(\\d+)\\s+(\\d+)(ST\\d{2}|316L|St\\.St)(\\d+(?:\\.\\d+)?)$',
          grup_haritasi: { adet: 1, tanim: 2, dn: 3, boy_mm: 4, kalite: 5, agirlik_kg: 6 }
        },
        {
