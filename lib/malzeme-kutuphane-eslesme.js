...
function eslesmeAnahtari(m, ARES_OLCU){
  ...
  if (/reducer/.test(tl)) {
    ...
    // 142: reducer çift çaplı ('219.1x6.3 / 114.3x4.5'). Büyük çap mm yukarıda (o.dis_cap).
    //   Küçük çap = '/' sonrası parça → ayrıca parse → lookup'ta cap_kucuk_mm ile daralt.
    //   Kütüphanede aynı büyük çapta birden çok küçük-çap varyantı var (duplicate DEĞİL) →
    //   tek çapla aramak belirsiz; küçük çap zorunlu ayraç.
    var kucuk_mm = null;
    var parcalar = String(m.boyut||'').split('/');
    if (parcalar.length >= 2){
      var ok = ARES_OLCU.olcuParse(parcalar[1], m.malzeme || '');
      if (ok && ok.dis_cap != null) kucuk_mm = Math.round(ok.dis_cap * 10) / 10;
    }
    return { hedef_kolon:'fitting_olculer_id', cap_alani:'cap_buyuk_mm', cap_mm:mm,
      cap_kucuk_alani:'cap_kucuk_mm', cap_kucuk_mm:kucuk_mm,
