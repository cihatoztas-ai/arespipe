...
function olcuZenginlestir(malzemeler) {
  ...
  for (const m of malzemeler) {
    ...
    if (typeof m.boyut !== 'string' || !m.boyut.trim()) continue;
    let p;
    try { p = O.olcuParse(m.boyut, _kaliteMalzeme(m.kalite)); } catch (e) { continue; }
    if (!p) continue;
    if (m.dis_cap_mm == null && p.dis_cap != null) m.dis_cap_mm = p.dis_cap;
    if (m.et_mm == null && p.et != null) {
      m.et_mm = p.et;
      m._et_kaynak = /[xX]/.test(m.boyut) ? 'metin' : 'asme';
    }
...
function spoolOlcuTuret(spool, malzemeler) {
  ...
  if (spool.dn == null && dom.dn != null) spool.dn = dom.dn;
