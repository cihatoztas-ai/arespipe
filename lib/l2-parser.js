...
function parse(text, parserKural) {
  ...
  // Alıştırma ipucu — not metninden config-driven türet (motor jenerik kalır, kelimeler kuralda)
  let alistirmaIpucu = null;
  const aik = parserKural.alistirma_ipucu_kurali;
  if (aik && aik.kaynak_alan && cikarilan[aik.kaynak_alan]) {
    const kaynak = String(cikarilan[aik.kaynak_alan]);
    const kelimeler = Array.isArray(aik.anahtar_kelimeler) ? aik.anahtar_kelimeler : [];
    const eslesti = kelimeler.some(k => { try { return new RegExp(k, 'i').test(kaynak); } catch (e) { return false; } });
    if (eslesti) alistirmaIpucu = aik.deger || 'KISMI';
  }
