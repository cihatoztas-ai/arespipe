...
// Boşsa doldurur (MK-111.2). et: metin-kaynaklıysa → et_kaynagi='tablo'.
// 165 REVİZYON (M130-817-008 vakası): 151'in "schedule-türetilmiş et spool'a yazılmaz,
// asmeFallbackDoldur schedule-bilinçli doldurur" varsayımı İKİ yönden çürük çıktı:
// (1) fallback helper yolu malzeme-kör (malzeme_en_kodu null -> 'karbon' -> 10S yok -> fail),
// (2) DB yolu schedule-kör (sorguda schedule_kod filtresi YOK, limit=1 -> SCH5/1.65).
// Satır-seviyesi asme et'i ise SATIRIN kalitesiyle (doğru malzeme) + basılı schedule'la
// hesaplandı -> daha güvenilir. Artık spool'a YAZILIR; etiket fallback'in birebir eşi
// ('ares_boru (SCH X)') -> tüketici yüzeyi aynı (MK-164.1 ruhu), fallback körlüğüne girilmez.
// izometri-oku'ya DOKUNULMADI (MK-49.1).
function spoolOlcuTuret(spool, malzemeler) {
  ...
  else if (dom.dis_cap_mm != null) {
    // 165 (AT110-804 vakası): DN basmayan ODxet çizimleri — dominant borunun dn'i yoksa
    // dis_cap'ten tablo-kesin ters eşleme (ARES_BORU.dnBul, tek-eşleşme şartı, uydurma yok).
    // Eşleşmezse (ASME-dışı OD) null kalır → dn_bulunamadi uyarısı haklı olarak sürer.
    const B = (typeof globalThis !== 'undefined' && globalThis.ARES_BORU) || null;
    const tersDn = B && B.dnBul ? B.dnBul(dom.dis_cap_mm, _kaliteMalzeme(dom.kalite)) : null;
    if (tersDn != null) spool.dn = tersDn;
  }
  if (spool.schedule == null && dom.sch) spool.schedule = dom.sch;
  if (spool.et_mm == null && dom.et_mm != null) {
    if (dom._et_kaynak !== 'asme') {
      spool.et_mm = dom.et_mm;
      spool.et_kaynagi = 'tablo';
    } else if (dom.sch) {
      // 165: satırın kalitesi + basılı schedule ile hesaplanmış asme et — fallback'ten güvenilir.
      spool.et_mm = dom.et_mm;
      spool.et_kaynagi = `ares_boru (SCH ${dom.sch})`;
    }
  }
}
