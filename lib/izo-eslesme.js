...
function seviyeBelirle(izo) {
  ...
  if (ek.includes('l2') || ek.includes('regex')) return 'L2';
 ...
 *
 * @returns {Object} {
 *   spoollar: [{pipeline, spoolNo, rev, anaMalzeme, toplamKg, yuzeyHam,
...
function incelemeTablosu({ kabukSpoollar = [], izometriler = [], guvenEsigi = 0.7 } = {}) {
  ...
  const bitenler   = izometriler.filter((z) => z.parse_durumu === 'tamamlandi');
  const isleniyor  = izometriler.filter((z) => z.parse_durumu !== 'tamamlandi').length;
  ...
  return {
    spoollar,
    fazla,
    ozet: { toplam: kabukSpoollar.length, okundu, zayif, eksik, fazla: fazla.length, isleniyor },
  };
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // Beklenen: özet {toplam:3, okundu:0, zayif:1, eksik:1, fazla:1, isleniyor:1}
