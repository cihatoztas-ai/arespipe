...
function incelemeTablosu({ kabukSpoollar = [], izometriler = [], guvenEsigi = 0.7 } = {}) {
  ...
  // 183/MK-183: PAOR kabuk anahtari cizim_no'dan (drawing-no). Tersan: cizim_no yok -> pipeline.
  const _kabukAnahtarKaynak = (sp) => (sp.cizim_no || sp.pipeline);

  // 188/A (MK-188.1): cizim_no -> o cizimde EN AZ BIR spool izometriyle eslesti mi?
  //   Operator override ile eklenen spool (S02..SN) icin PDF AYNI cizimin kendisi (model az okudu,
  //   operator dogru sayiyi girdi). Bu spool'un kendi spoolNo anahtari harita'da YOK -> eski mantik
  //   'eksik' (doküman yok) basardi = YANLIS damga. Cozum: ayni cizimde kardes spool eslestiyse,
  //   override-eklenen spool'u 'zayif' say + temsil izometriyi kardesten DEVRAL. Format-agnostik.
  const _cizimDevral = new Map();   // _norm(cizim_no) -> { izo, izoIdx }
  bitenler.forEach((z, idx) => {
    (z.anahtarlar || []).forEach((a) => {
      const ck = _norm(a.pipeline);
      if (!ck) return;
      if (!_cizimDevral.has(ck)) _cizimDevral.set(ck, { izo: z, izoIdx: idx });
    });
  });

  const spoollar = kabukSpoollar.map((sp) => {
    const k = anahtar(_kabukAnahtarKaynak(sp), sp.spoolNo);
    const eslesmeler = harita.get(k) || [];

    if (eslesmeler.length === 0) {
      // 188/A: dogrudan eslesme yok — ama OVERRIDE shell (_paorBolShell, _malzeme_devralindi) VE
      //   ayni cizimde kardes spool eslestiyse PDF mevcut, devral. Guard: yalniz override shell'ler.
      //   Tersan'da S01/S02 ayni pipeline anahtarini paylasir ama override DEGIL -> devralma YOK,
      //   gercek eksik izometri 🔴 kalir (B-6 korunur). _malzeme_devralindi shell'de set edilir.
      const ck = _norm(_kabukAnahtarKaynak(sp));
      const dev = (sp._malzeme_devralindi === true && ck) ? _cizimDevral.get(ck) : null;
      if (dev) {
        eslesenIzoIdx.add(dev.izoIdx);
        zayif++;
        return {
          pipeline: sp.pipeline, spoolNo: sp.spoolNo, rev: sp.rev,
          anaMalzeme: sp.anaMalzeme, toplamKg: sp.toplamKg, yuzeyHam: sp.yuzeyHam,
          kalite: (sp.kalite != null ? sp.kalite : null),
          cap: (sp.cap != null ? sp.cap : null), et: (sp.et != null ? sp.et : null),
          durum: 'zayif',
          izometri: {
            dosya_adi: dev.izo.dosya_adi,
            seviye: seviyeBelirle(dev.izo),
            guven: (typeof dev.izo.guven === 'number') ? dev.izo.guven : null,
          },
          bindirme_flag: false, override_kardes: true,
        };
      }
      eksik++;
      return {
  ...
  // 184/A (MK-184.2): Kabuk anahtar seti — fazla + montaj ortak kullanir (tani: tek tanim).
  const kabukAnahtarSet = new Set(kabukSpoollar.map((sp) => anahtar(_kabukAnahtarKaynak(sp), sp.spoolNo)));

  // FAZLA (🟠): kabukta KARSILIGI OLMAYAN izometri anahtarlari + anahtari cikarilamayanlar.
  //   Sessizce eklenmez — kullaniciya "ekle/yoksay" diye SORULUR (MK-126.2).
  // 184/A: ANAHTAR-SEVIYESI (eski izometri-seviyesi DEGIL). Cok-spool PDF'te bir spool (S01) kabuga
  //   tutarsa digerleri (S02..SN) ESKIDEN sessizce yutuluyordu (B-6 ihlali) -> simdi her eslesmeyen
  //   anahtar ayri fazla. PAOR pozisyon dali = tek cok-anahtar ureteni; Tersan tek-anahtar -> birebir
  //   ayni (anahtar setteyse fazla yok, degilse fazla). Format-agnostik: 'if paor' yok.
  const fazla = [];
  bitenler.forEach((z) => {
    const anahtarlar = z.anahtarlar || [];
    if (anahtarlar.length === 0) {
      // Anahtar cikmadi (or. dosyaAdiParse null + parse pipeline yok) → MK-124.1 borcu YUZEYE CIKAR.
      fazla.push({
    ...
    anahtarlar.forEach((a) => {
      if (kabukAnahtarSet.has(anahtar(a.pipeline, a.spoolNo))) return;   // kabukta var -> fazla degil
  ...
  //   (kabukAnahtarSet 184/A'da fazla blogundan ONCE tanimlandi — tek tanim.)
  ...
  // 188/A (MK-188.2): stabil sirala — (pipeline) sonra spoolNo (S01->S02->S03 ardisik).
  //   _paorBolShell override spool'u concat ile sona ekledigi icin liste dagilik goruluyordu.
  spoollar.sort((a, b) => {
    const pa = String(a.pipeline || ''), pb = String(b.pipeline || '');
    const pc = pa.localeCompare(pb, undefined, { numeric: true, sensitivity: 'base' });
    if (pc !== 0) return pc;
    return String(a.spoolNo || '').localeCompare(String(b.spoolNo || ''), undefined, { numeric: true, sensitivity: 'base' });
  });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // ── 184/A VAKASI — cok-spool kismi eslesme (PAOR pozisyon) + Tersan tek-anahtar regresyon.
  //   PAOR: cizim 102769 L3'te 2 spool (anahtarlar S01+S02), kabukta yalniz S01 -> S02 FAZLA olmali
  //   (eskiden izometri-seviyesi mantik S01 tutunca S02'yi sessizce yutuyordu = B-6). Tersan: tek
  //   anahtarli izometri -> eski davranis (eslesirse fazla yok, eslesmezse fazla) BIREBIR korunur.
  const kabuk2 = [
    // PAOR kabuk: eslestirme anahtari cizim_no (drawing-no), ekranda pipeline Z05-... kalir
    { pipeline: 'Z05-SCUPPER_002', cizim_no: '52600-102769', spoolNo: 'S01', rev: '', anaMalzeme: 'ST37', toplamKg: 10, yuzeyHam: '' },
    // Tersan kabuk: cizim_no yok -> anahtar pipeline'dan
    { pipeline: 'G400-804-604', spoolNo: 'S01', rev: '', anaMalzeme: 'ST37', toplamKg: 12, yuzeyHam: '' },
  ];
  const izo2 = [
    // PAOR fab PDF: pozisyon anahtarlari S01+S02 (drawing-no pipeline). S01 kabukta, S02 YOK.
    { dosya_adi: '11D-PAOR-52600-102769-A.pdf', parse_durumu: 'tamamlandi',
      anahtarlar: [{ pipeline: '52600-102769', spoolNo: 'S01' }, { pipeline: '52600-102769', spoolNo: 'S02' }],
      et_kaynagi: 'l2_regex', guven: 1 },
    // Tersan: tek anahtar, kabukta var -> fazla YOK (regresyon)
    { dosya_adi: 'G400-804-604.S01.1.pdf', parse_durumu: 'tamamlandi',
      anahtarlar: [{ pipeline: 'G400-804-604', spoolNo: 'S01' }], et_kaynagi: 'l2_regex', guven: 1 },
  ];
  const r2 = incelemeTablosu({ kabukSpoollar: kabuk2, izometriler: izo2 });
  const s02 = r2.fazla.filter((f) => f.spoolNo === 'S02' && f.pipeline === '52600-102769');
  const tersanFazla = r2.fazla.filter((f) => f.pipeline === 'G400-804-604');
  console.log('\n--- 184/A: cok-spool kismi eslesme ---');
  console.log('  okundu:', r2.ozet.okundu, '(2 bekleniyor: PAOR S01 + Tersan S01)');
  console.log('  fazla :', r2.ozet.fazla, '| S02 fazlada:', s02.length === 1, '| Tersan fazla:', tersanFazla.length, '(0 bekleniyor)');
  const ok2 = r2.ozet.okundu === 2 && s02.length === 1 && tersanFazla.length === 0 && r2.ozet.fazla === 1;
  console.log('184/A SELF-TEST:', ok2 ? '✅ GEÇTİ' : '❌ KALDI');

  // ── 188/A VAKASI — operator override + sibling izometri devralma + siralama.
  //   PAOR cizim 52600-102769: model 1 spool okudu (izometri anahtari YALNIZ S01). Operator 2 dedi ->
  //   _paorBolShell sibling S01'den S02 shell uretti (malzeme devralindi). incelemeTablosu: S02'nin kendi
  //   anahtari harita'da YOK ama ayni cizimde S01 eslesti -> S02 'zayif' (override_kardes), 'eksik' DEGIL.
  const kabuk3 = [
    { pipeline: 'Z05-SCUPPER_002', cizim_no: '52600-102769', spoolNo: 'S01', rev: '', anaMalzeme: 'ST37', toplamKg: 10, yuzeyHam: '' },
    // override ile eklenen shell (_paorBolShell: _malzeme_devralindi + malzeme S01'den, toplamKg 0)
    { pipeline: 'Z05-SCUPPER_002', cizim_no: '52600-102769', spoolNo: 'S02', rev: '', anaMalzeme: 'ST37', toplamKg: 0, yuzeyHam: '', _malzeme_devralindi: true },
  ];
  const izo3 = [
    // model YALNIZ S01 okudu (override oncesi gercek parse)
    { dosya_adi: '11D-PAOR-52600-102769-A.pdf', parse_durumu: 'tamamlandi',
      anahtarlar: [{ pipeline: '52600-102769', spoolNo: 'S01' }], et_kaynagi: 'l2_regex', guven: 1 },
  ];
  const r3 = incelemeTablosu({ kabukSpoollar: kabuk3, izometriler: izo3 });
  const s01_3 = r3.spoollar.find((s) => s.spoolNo === 'S01');
  const s02_3 = r3.spoollar.find((s) => s.spoolNo === 'S02');
  console.log('\n--- 188/A: override sibling devralma + siralama ---');
  console.log('  S01:', s01_3 && s01_3.durum, '(okundu bekleniyor)');
  console.log('  S02:', s02_3 && s02_3.durum, '| override_kardes:', s02_3 && s02_3.override_kardes,
              '| izo:', s02_3 && s02_3.izometri && s02_3.izometri.dosya_adi);
  console.log('  ozet eksik:', r3.ozet.eksik, '(0 bekleniyor) | fazla:', r3.ozet.fazla, '(0 bekleniyor)');
  console.log('  sira:', r3.spoollar.map((s) => s.spoolNo).join(' -> '), '(S01 -> S02 bekleniyor)');
  const ok3 =
    s01_3 && s01_3.durum === 'okundu' &&
    s02_3 && s02_3.durum === 'zayif' && s02_3.override_kardes === true &&
    s02_3.izometri && s02_3.izometri.dosya_adi === '11D-PAOR-52600-102769-A.pdf' &&
    r3.ozet.eksik === 0 && r3.ozet.fazla === 0 &&
    r3.spoollar[0].spoolNo === 'S01' && r3.spoollar[1].spoolNo === 'S02';
  console.log('188/A SELF-TEST:', ok3 ? '✅ GEÇTİ' : '❌ KALDI');
}
