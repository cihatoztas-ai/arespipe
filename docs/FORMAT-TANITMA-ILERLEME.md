...
## Oturum 162 (2026-06-06) — METİN AYRIŞMASI kök bulgusu + pipeline_no kuralı GEMİDE + ATÖLYE KARARI (MK-162.1)
1. **Y200 öğretimi kayda dönemedi ama oturumu BÜYÜTTÜ:** pipeline_no'da kural/AI/UI üç farklı
   değer verdi (G200/Y110/Y200) → elden geçirme (Cihat önerisi). Kök: format_tanit `extractAll`
   GÖRSEL sıra (pdf.js Y-gruplama), sunucu content-stream sırası — **tarayıcı metni ≠ sunucu
   metni.** UI testi bu sınıf alanlar için kanıt değil. Ayrıca `cozumle()` oto-çözüm yolu synth
   çapa disiplinini bypass ediyor (DB'deki eski çıplak `(Y\d+...)` kuralının kaynağı) ve Cadmatic
   şablonunda "etiketin ardından" YAPISAL ÖLÜ (değerler etiket bloğundan önce basılıyor).
2. **pipeline_no kuralı KANITLI + GEMİDE:** sunucu metni lokal dökümle (pdf-parse+glyph-onar)
   mekanik test → `\n([A-Z]\d+-\d+-\d+)\n` = Y200-804-414; a093eaaa'ya jsonb_set ile yazıldı
   (şema-önce SELECT → UPDATE → doğrulama SELECT). `[A-Z]` bilinçli: bölge kodu ≠ format
   (Cihat — aynı desen başka bölgelerde, aynı bölgede başka desenler olabilir).
3. **Hibrit kayıt sırası dersi:** SQL-önce yazılan alana UI'da DOKUNULMAZ (dirty → Güncelle
   ezer); `_patchedKural` derin-kopya + yalnız-dirty (MK-111.2) — dokunulmadıkça SQL güvende.
4. **otoTespit açığı ÜÇÜNCÜ kez tetiklendi (E100-817-005):** yine "tersan deneme" (5/6), modal
   satır-deseni kaydına bir tık kaldı. Tablo içerik dili de farklıydı (E100: NPS+Sch `2" Sch
   10S/316L`; Y200: ODxet `139.7x4.5`). Şans: Güncelle "değişiklik yok" dedi, DB temiz
   (satir_tipi_sayisi=0 teyitli).
5. **YENİ ŞÜPHE — B6:** "değişiklik yok" aynı zamanda tablo öğretiminin düzeltme-kipi YAZIM
   yolunun çalışmadığı anlamına gelebilir (`_patchedKural` `a.tip!=='tablo'` filtreli; tablonun
   ayrı yazım yolu doğrulanmadı). Y200 kaydının 1 numaralı kontrol maddesi (reçete:
   docs/FORMAT-OGRETIM-ATOLYE-162.md).
6. **MK-162.1 [URUN] — ATÖLYE KARARI (Cihat):** format_tanit operatör ürünü DEĞİL, uzman
   aracı. Menü girişi kaldırıldı (`6a27723`); sayfa feature-flag'le yaşar, batch Tanıt köprüsü
   uzman aracı olarak kalır. Format öğretimi Cihat+Claude atölyesi: zip → sunucu-metni dökümü →
   kural türetimi (mekanik kanıt) → SQL kayıt → drenaj kanıtı. Kullanıcıya açılma şartı: C yolu
   (otomatik kural önerisi + onay). MK-162.2: G2a düzeltme birikimi → kural ÖNERİSİ köprüsü
   (Düzelt=değer/Tanıt=kural ayrımı korunur; birikim öneriye, öneri onaya).
7. **Wizard soru sadeleşmesi (Cihat, kabul):** cap/et/dn ayrı öğretilmez — malzeme sentezinden
   türer (W-3.9 zaten zorluyor). Öğretim çekirdeği: KİMLİK + MALZEME BÖLGESİ (+opsiyoneller).
8. **MK-85.3 İKİNCİ ihlal (Claude, kabul):** `is_kuyrugu.format_id` ezberden — şema gerçeği:
   format_id/parser_seviye/maliyet/pdf_sha256 **ai_api_log** kolonları; kuyruk `ai_api_log_id`
   ile bağlanır. Kanıt JOIN şablonu atölye dokümanında. E100-817 is_kuyrugu'da YOK (bilgi).
Format hattı kuyruğu: Y200 kaydı reçeteyle DİĞER bilgisayardan yapılabilir (Cihat); 163 yönü
YAPISAL eksikler (B6 + kuyruk drenajı + motor kopyası), format ÇEŞİTLİLİĞİ sonraya (Cihat kararı).

## 163. OTURUM (2026-06-06) — yapısal temizlik: B6 hükmü, B7 bitti, B4 ölçüldü, kimlikler düzeldi
1. **B6/W-3.11 HÜKMÜ:** tablo yazım yolu SAĞLAM (152 köprüsü yerinde). Gerçek kapı:
   `_tabloSentezle` yalnız `yesil>0` ise `_satirTipleri` set eder ve sentez CANON_ALL
   (tarayıcı/görsel-sıra) üzerinde koşar — B1 metin ayrışmasının tablo hattındaki izdüşümü.
   Bonus kusur: `tamamlaAc` düzelt kipinde tablo patch'ini önizlemeye KOYMUYORDU (önizleme ≠
   yazım) — 162'deki "modal gösterdi ama yazılmadı" izlenimi büyük olasılıkla buydu. Patch:
   D1 ayrıştırılmış "değişiklik yok" toast'ı · D2 `_tabloYeniMt/_tabloDegistiMi` tek kaynak
   (önizleme=yazım) · D3 `_patchedKural` W-3.9 kapısı. 13/13 mekanik test. **Reçete eki:**
   Güncelle'den önce yeşil sentez kutusu görülmeden basılmaz.
2. **W-3.12 — motor TEK KAYNAK:** `_alanCikar/_postProcess` kopyaları silindi; çekirdek
   `ares-alan-cikar.js` (kök IIFE), l2-parser ince delege (izometri-oku import'u etkilenmez —
   imzalar aynen). Kopyada whitelist'e ek format_template dalı da eksikti. Kanıt: git-HEAD
   gövdesi == çekirdek 13/13 + canlı delegasyon. F1 taraması: 27 kuralda fallback/whitelist/
   format_template SIFIR → B4 sessiz-fallback teorik (tek yerden eklenecek).
3. **Format kimlikleri (B3, B5 kökü):** a093eaaa = **"Tersan Cadmatic Spool — Öğretim
   (çok-notasyon)"**, kod **`tersan_cadmatic_spool_ogretim_v1`** ("tersan deneme"/"nps_v1"
   tarihe karıştı — eski adla ARAMA YAPMA). e1fb879d/39a2c81b "— Katalog" ekli yapısal adlara.
   B5'in kökü: a093eaaa ↔ e1fb879d aynı yapısal aile, iki ayrı BAŞLIĞA parmak basıyorlar
   ("Cut & Bending Info" ↔ "Malzeme Listesi"); ayrım KAYNAK (öğretim-DB vs katalog-paket).
   pipeline_no kuralı (`\n([A-Z]\d+-\d+-\d+)\n`) ad/kod değişiminde DOKUNULMADI (doğrulamalı).
4. **G2a köprüsü v1 (MK-162.2):** migration 102 görünümü (3+ birikim sinyali,
   security_invoker=true) + uyarilar 📐 kutusu. Düzelt=değer/Tanıt=kural KORUNUR.
5. Hayalet borç dersi (MK-163.1): "kuyruk 100+ stuck" ve MK-117, 153-155'te çözülmüştü —
   163 SQL denetimi teyit etti; devir borçları taşınmadan tazelenir.
Format hattı kuyruğu: Y200 malzeme kaydı + W-3.4 yayılım Cihat'ta (diğer bilgisayar, reçete +
163 eki); zip atölyesi (çeşitlilik) ondan sonra. OPR (f) canlı kanıtı da Cihat'ta.


---

## 164. OTURUM (2026-06-06, akşam) — G2a kaynak bağı + W-2.19 ucuz dilim + kalite alanı
1. **164-B1 BULGUSU + G2a v2 (A kararı):** v1 sinyallerinin (7 kart) değer kaynağı Excel
   kabuktu — v1 kart metni "PDF format kuralı şüphesi" diyerek YANLIŞ adrese yolluyordu.
   Migration 103: `taslak_duzeltmeleri.deger_kaynagi` (excel/izometri/operator, CHECK) +
   `format_id` (FK) + görünüm v2 kaynak kırılımlı. Eski satırlar NULL='bilinmiyor' — tahmin
   backfill YOK. Kart metni kaynak-dallı (Excel hattı / PDF formatı "AD" / operatör / bilinmiyor).
2. **MK-164.1:** ilk yamadaki sabit `'excel'` ekran kanıtıyla (L2 rozetli alanlar) çürüdü →
   v2.1 alan-bazlı kaynak (rozet kuralının birebir eşi) + devre-inceleme `ps.format_id` taşır.
   **Ders: rozet VARSAYILANI ≠ kaynak KURALI — yazan kod, gösteren kuralın eşini kullanır.**
3. **W-2.19 ucuz dilim + W-4.4:** bbox yakalama anında scale-1 normalize (`norm:1`);
   🔍 Tablo butonu konum_ipucu'dan (yalnız norm) dpvZoomTo+vurgu. Norm'suz kayıt kullanılmaz.
   Y200 tablo öğretimi yapıldığında buton otomatik canlanır — **reçeteye eklendi.**
4. **Kalem kalite alanı (Cihat):** dinamik süzgeçli dropdown + kabuk `dz.kalite` tercihi;
   OPR'nin `kalite=malzeme` saçması bitti.
5. **MK-163.6 kapandı:** parse_durumu yumuşak emeklilik (ölü okuma silindi + BAYAT COMMENT).
6. **YENİ (165):** M130-817-008 çap 42.2/et 3.56 — format **e1fb879d KATALOG-PAKET** (kuyruk
   SQL kanıtı). Paket cap/et seçim kuralı şüphesi (redüksiyon küçük ucu hipotezi); parse_sonuc
