...
## MK-185.1 — Kapsam-etiketli malzeme görünümü (imalat/montaj/işlem çip)
Spool malzeme listesinde her kalem iş-akışı kapsamına göre etiketlenir: imalat (boru/dirsek/te/flanş/bilezik-stub/doubler/sleeve — kaynakla birleşen, kesim/büküm/markalama bunlarla çalışır), montaj (vana/strainer/civata/somun/conta/rondela — sahada eklenen), işlem (yiv/kaynak). ares-normalize.kapsamEtiket(tanim) global helper (TEK kaynak — malzeme-kiyas.js kapsam'ından FARKLI; o PDF↔Excel kıyas beklentisi, flanş orada montaj; bu iş-akışı, flanş burada imalat). spool_detay'da 3 çip filtresi (default: imalat+işlem açık, montaj kapalı), montaj/işlem rozeti. Veri SİLİNMEZ, yalnız filtrelenir → montaj-ekibi versiyonunda hazır. Tersan'a sıfır risk (montaj=0 çip soluk, işlem dolu olur, imalat liste birebir). Kesim/büküm zaten izinli=['boru']/['flans','bilezik'] ile korumalı — montaj sızmaz. Push: f7936ff.

## MK-185.2 — PAOR L3 spool-sayma prompt güçlendirme (GEÇİCİ — 186'da kompozisyona taşınır)
Varsayılan YAKLASIM_Y_PROMPT madde 3 yalnız "[1][2]=2 spool" örneği veriyordu → model 2'ye şartlandı, 3-spool çizimde (102782 SPOOL [1][2][3]) üçüncüyü kaçırdı (taze L3 de 1 okudu, cache DEĞİL). 6 çizimden 5'i (2 spool) doğru, tek 3-spool yanlış = few-shot şartlanması imzası. Çözüm: paor_aveva_ana (995b5514) prompt_template'ine varsayılanın TAMAMI + güçlendirilmiş madde 3 (2/3/4/5+ örnekleri, SPOOL kutusu vurgusu, <> vs [] ayrımı). Migration 105 (E-string tek satır). izometri-oku.js DOKUNULMADI (formatBilgisi.prompt_template:721 override eder). Canlı kanıt: 773+782 → 3 spool. NOT: Bu OVERRIDE geçici; 186'da kompozisyona (evrensel çekirdek + format ek) taşınınca geri alınır.

## MK-185.3 — Prompt & cache mimarisi (TASARIM BORCU → 186)
Halı-altı tespiti (Cihat): her format izole prompt_template (override) + her prompt değişiminde elle cache temizliği → N format ölçeklenmez. İki yapısal boşluk: (1) prompt override (evrensel kural yayılmaz, drift), (2) cache anahtarında prompt sürümü yok (değişince eski cache donar, köstebek oyunu — 185'te ~3 saat). Hedef mimari: prompt override→KOMPOZİSYON (evrensel çekirdek + format-özel ek; spool-sayma çekirdeğe → tüm formatlar+gelecekler faydalanır) + cache anahtarına prompt sürümü (otomatik MISS, elle temizlik biter). Parser tarafında katman kompozisyonu ZATEN var (format-paketleri Katman 0-3) — prompt'ta yok, tutarsızlık. Engel: MK-49.1 (izometri-oku dokunulmaz) — çözüm lib/prompt-birlestirici.js (katman-birlestirici modeli, izometri-oku ince kalır). Tam spec: docs/186-PROMPT-CACHE-MIMARI-SPEC.md. 185'te son manuel temizlik: 46 PAOR Ana cache invalidate (format_id scoped, Tersan'a sıfır dokunuş, DRY kanıtlı).

## MK-186.1 — Prompt override → KOMPOZİSYON + ölü kod temizliği (185.3 borcu kapatıldı)
185'in GEÇİCİ override'ı (her format kendi prompt_template'i) kalıcı kompozisyona taşındı. `lib/prompt-birlestirici.js`: `EVRENSEL_PROMPT` (= eski YAKLASIM_Y_PROMPT + güçlendirilmiş madde 3, 7025 char) + `promptBirlestir(formatBilgisi)` = EVRENSEL + (format.`prompt_ek` varsa). Kilit kanıt: migration 105 prompt'u = YAKLASIM_Y + YALNIZ güçlendirilmiş madde 3 (prefix/suffix birebir) → PAOR'a özgü kural YOK → PAOR `prompt_ek` BOŞ (saf evrensel). 25/25 birim test (Tersan regresyon mekanik kanıtlı). `izometri-oku.js:visionAIParse` artık `promptBirlestir(formatBilgisi)` çağırır (override satırı kalktı). MK-49.1 esnetme: prompt mantığı lib'de, izometri-oku ince. Migration 106 `prompt_ek` kolonu (additive). **Step 7 (temizlik):** ölü `YAKLASIM_Y_PROMPT` sabiti izometri-oku'dan SİLİNDİ (140 satır), migration 108 ile mig105 `prompt_template` geri alındı (NULL'landı, id-scoped 995b5514). Regresyon YOK: güçlendirilmiş madde-3 EVRENSEL_PROMPT'ta baki; `istek_surum` prompt_template kullanmaz → cache geçerli kaldı. MK-155.1 tuzağı (DB prompt_template runtime'da yok sayılır) kapandı. Commit Step 7: 332827e.

## MK-186.2 — Vision çağrısı temperature: 0 (varyans fix)
visionAIParse istek'inde `temperature` set edilmemişti (varsayılan 1.0) → AYNI çizim run-to-run 3↔1 zıplıyordu (782 kanıtı: aynı 4821-token prompt, 12:51=3 / 14:30=1 = saf varyans, kod değil). `temperature: 0` eklendi → spool-sayma ekstraksiyonu deterministik (aynı PDF+sürüm hep aynı sonuç). NOT: temp değişirse PARSE_SURUM elle artırılmalı (cache sürüm hash'inde temp ayrı tutulmuyor, PARSE_SURUM'a foldlandı).

## MK-186.3 — SPOOL kutusu zoom kırpımı → L3 2. görsel (eksik-sayım KÖK çözümü)
Asıl kök neden çözüldü: model tam-sayfa A3'te küçük `[n]` parantezlerini KAÇIRIYOR (780/782: gerçek 3, model 1) — prompt değil, ÇÖZÜNÜRLÜK sorunu (Claude SPOOL kutusunu zoom'layınca [1][2][3]'ü anında okudu). Çözüm: `ares-izometri-drenaj.js` (frontend, mevcut `vendor/pdfjs-1.10.100` ile) PDF'in SAĞ malzeme kolonunu (`_SPOOL_KIRP={x0:0.44,y0:0,x1:1.0,y1:0.62,hedefGen:1500}`) yüksek-DPI render→crop→PNG base64, `spool_kirpim_b64` olarak POST'a ekler. Hata→null (graceful, parse bozulmaz). `izometri-oku.js`: varsa 2. görsel (image) + `KIRPIM_TALIMATI` (odaklı sayma: "[n] koşeli say, <> kesim değil"). **Tek L3 çağrısı** (timeout yok), **yeni endpoint yok** (12/12). Client-side render → MK-113/A korunur. CANLI KANIT: 780 14:30 `spool=1,input_tokens=4821` (crop yok) → 17:21 `spool=3,input_tokens=6582` (crop). Yeni 769-778 seti (KABUK 20): model **10/10 TAM doğru** (yüksek-zoom yer-hakikatiyle teyit; Claude'un düşük-res montaj ilk okumaları 769/775'te HATALIYDI, model haklı çıktı — varsayım değil veri). Commit: 543711d. KIRMIZI ÇİZGİ: PAOR fab-PDF; Tersan L2-first, dokunulmaz.

## MK-186.4 — Cache sürümleme (istek_surum) — köstebek oyunu KALICI bitti
185-186'nın asıl acısı (prompt/crop değişince eski cache donar, elle invalidate dansı) kökten çözüldü. `istek_surum` = sha256(`PARSE_SURUM`|model|`promptBirlestir(formatBilgisi)`|crop-modu+KIRPIM_TALIMATI)[:16] → cache anahtarına girer. `cacheKontrol` artık `&istek_surum=eq.X` ile eşler; eski (NULL) kayıtlar yeni sürüme eşleşmez → MISS → bir kerelik temiz tazeleme, sonra stabil. Migration 107 `ai_api_log.istek_surum` kolonu (additive). `KIRPIM_TALIMATI` + crop talimatı TEK KAYNAK (sabit) → istek ile hash birebir. CANLI KANIT: 769-778 seti iki kez yüklendi → 13/13 çizim `l3_odeme=1` (ikinci yükleme YENİ L3 yazmadı = HIT, $0). Prompt/crop değişince PARSE_SURUM/hash otomatik değişir → MISS. Commit: ad45644. Artık manuel `UPDATE basarili=false` invalidate GEREKMİYOR.

## MK-187.1 — Sessiz-fallback emniyet ağı (B): crop yok + PAOR → manuel_onay
SPOOL kutusu zoom kırpımı (186.3) yalnız L3 vision'da kullanılır. Crop render istemcide patlarsa drain `spool_kirpim_b64: null` gönderir → izometri-oku SESSİZCE crop'suz tam-sayfa parse eder → model küçük [n]'leri kaçırıp eksik sayabilir, ALARM YOKTU. Çözüm: izometri-oku visionAIParse'ta `_sayim_kirpimsiz = !spool_kirpim_b64` üretir, cevap_full'a yedirilir (cache'te yaşar, kirpimsiz parse cache'ten gelirse de taşınır) ve yanıt özetine konur. kuyruk-isle-izometri durum kararı: `okuJson._sayim_kirpimsiz === true && dosyaAdiParse(dok.dosya_adi)?.spool_kaynak === 'pozisyon'` → çizim `manuel_onay`'a ("gözle say") + `parse_sonuc._sayim_uyarisi`. KİLİT İLKE: B yalnız sayının PDF-pozisyon kaynaklı olduğu (PAOR; Excel'de spool listesi YOK) durumda anlamlı; spool_kaynak≠'pozisyon' (Tersan / Excel-listeli) → liste otoriter, gözle-say gereksiz → yoksayılır. Tek başına manuel_onay'a düşürmez. Tersan kırmızı çizgisi korunur. Migration/api yok, 12/12. Commit: 1afe670.

## MK-187.2 — PAOR çizim-başı SPOOL göz-onayı (A1): görsel çetele, crop'u depolamadan yeniden-üret
B otomatik olarak #2'yi (sessiz crop-fail) kapatır; A1 ise crop GİTSE bile model yanlış saydığında (#1 crop bölgesi yanlış / #3 model crop'la bile yanlış saydı) operatörün GÖZLE yakalamasını sağlar. Dökümanlar sekmesinde çizim-başı: kırpılan SPOOL kutusu thumbnail + "PDF'ten N spool" + S0n markaları. KİLİT KARAR: crop'u parse anında DEPOLAMAK yerine istemcide `_spoolKirpim` ile YENİDEN ÜRET — fonksiyon deterministik (aynı PDF + aynı `_SPOOL_KIRP` bölgesi → modelin gördüğü görüntünün AYNISI). Sonuç: $0 depolama, migration yok, parse/drain yoluna (186 cache + Tersan kırmızı çizgisi) dokunmadan, GERİYE-DÖNÜK (tüm mevcut PAOR devreleri re-parse'siz çalışır). `_spoolKirpim` drain public API'ye eklendi (`ARES_IZO_DRENAJ.spoolKirpim`, MK-109.1 — kopyalama değil expose). Thumbnail cache (override sonrası re-render PDF'i tekrar çekmesin). YALNIZ PAOR (`-PAOR-` deseni). Commit: 87dc444.

## MK-187.3 — PAOR spool sayısı operator override (A2 dilim-1)
Cihat senaryosu: "PDF 3 spool için çizilmiş ama model 1 saydı → operatör S02/S03 ekleyip PDF okumuş gibi devam etsin." 1→N genişletme mekanizması ZATEN var (`_paorBolShell`, 184/A MK-182.6): `/api/devre-inceleme`'den gelen `fazla` (PDF'in bulduğu, kabukta-olmayan spool'lar) için boş shell enjekte eder. A2 = operatörün bu fazla'yı ELLE söylemesi. A1 panelinde "Gerçek spool sayısı" girişi + Uygula → sentetik fazla (S0k, sebep:'kabukta_yok') üret → `_paorBolShell(WIZ._kabukSpoollar, synth)` → `inceleGetir`. Terfi-öncesi katman (WIZ._kabukSpoollar bellekte); kabuk mutasyonu yok, terfide (ARES_KABUK.aktar, idempotent) kesinleşir → çift kayıt riski yok. YALNIZ PAOR (cizim_no `-PAOR-` deseninden). Commit: c93d66e.

## MK-187.4 — A2 override idempotent fix: katlanma bug'ı
CANLI BUG (NB138): 102769 PDF=2 spool, ama Uygula sonrası kabukta 5-7 (S01..S07) birikti — üst üste ekleme. ÜÇLÜ kök sebep: (1) `_a2SayiYukselt` "N'e kadar EKLE" mantığındaydı (simdi+1..hedef) → tekrar/farklı değer girince katlanma; (2) `inceleGetir` kendi içinde `_paorBolShell`'i server fazla'sıyla TEKRAR çalıştırıyordu (ikinci kat); (3) panel "bir kez render" guard'ı + panel sayısı PDF-bağlı spool'lardan → override (izometrisiz shell) panelde görünmüyor → "değişti mi?" belirsizliği → operatör tekrar basıp katladı. ÇÖZÜM: `_a2SayiYukselt` IDEMPOTENT "N'e AYARLA" (hedef>simdi: ekle; hedef<simdi: S0(hedef+1)..S0(simdi) çıkar) → tekrar bassa da sonuç=N; `inceleGetir(bolAtla)` parametresi — override'tan çağrılınca `_paorBolShell` re-expansion ATLANIR; panel guard kaldırıldı + `inceleGetir` sonunda (dok sekmesi açıkken) panel tazelenir; panel artık "PDF'ten N" + (override varsa) "Düzeltildi → kabukta M", giriş kutusu = kabuk gerçeği. DERS: override idempotent (SET) olmalı + sessiz başarı = başarısızlık gibi görünür (geri-bildirim mantık kadar kritik). Commit: 0e50dd3.
