...
`window.t()` → `window.tv()`'ye geçildi. `t()` artık `tv()`'yi çağıran bir alias. NAV render ve `applyLang()` `window.tv()` kullanıyor.

### 3.6 vercel.json — Cache Kontrolü ✅

`ares-lang.js`, `ares-store.js`, `ares-layout.js`, `lang/*.json`, tüm `.html` dosyaları — no-cache mevcut.
...
|---|---|---|---|---|---|
| `giris.html` | ✅ | — | ✅ | — | Temiz |
| `index.html` | ✅ | ✅ | ✅ | ✅ | Temiz, widget sistemi mevcut |
| `devreler.html` | ✅ | ✅ | ✅ | — | Temiz |
...
| `qr_tara.html` | ✅ | ✅ | ✅ | ✅ | Temiz, rol yönlendirmesi mevcut |
...
### 5.3 Silinmiş Sayfalar ✅

- `personel.html` — silindi
- `personel_yeni.html` — silindi
- `senaryolar.html` — silindi
...
- [x] `qr_tara.html` → rol bazlı yönlendirme + dil sistemi
- [x] `spool_detay.html` → URL param fix + `tarama_sonuclari` kaldırıldı
- [x] `vercel.json` → no-cache tüm kritik dosyalar
- [x] `en.json` + `ar.json` → `nav_personel` silindi
- [x] `ares-layout.js` → sidebar aktif sayfa tespiti iyileştirildi
- [x] `ares-layout.js` → `t()` → `tv()` alias geçişi

### FAZ 5 — Silme ✅ TAMAMLANDI
- [x] `personel.html` silindi
- [x] `personel_yeni.html` silindi
- [x] `senaryolar.html` silindi
- [x] `ares-layout.js` NAV listesinden personel kaldırıldı

### FAZ 6 — İşlevsel geliştirme
- [ ] `ayarlar.html` → `tenants` Supabase bağlantısı
- [x] `qr_tara.html` → rol bazlı yönlendirme ✅
- [x] `ares-layout.js` → `t()` → `tv()` geçişi ✅
- [x] `ares-layout.js` → default tema `light-anthracite` ✅
- [x] `ares-layout.js` → topbar canlı saat eklendi ✅
- [x] `index.html` → stat kartları Supabase'den yükleniyor ✅
- [x] `index.html` → widget sistemi (mini/orta/büyük, drag&drop, localStorage+Supabase) ✅
- [ ] `is_baslat.html` → yeni sayfa (operatör saha ekranı)
...
### Widget Sistemi — index.html
3 widget, her biri bağımsız boyutlandırılır ve sıralanır:

| Widget | Mini | Orta | Büyük |
|---|---|---|---|
| Stat kartları | Tek satır sayılar | — | Tam kartlar |
| Spool | Sadece toplam sayı | Donut grafik | Donut + çap tablosu |
| Takvim | Kompakt ay görünümü | — | Tam takvim |

- Drag & drop ile sıra değiştirme
- ✕ ile gizle/göster
- Layout `localStorage` + `kullanicilar.ui_tercihleri` Supabase kolonuna kaydedilir
- Supabase kaydı sayesinde cihazlar arası senkron

### ui_tercihleri Kolonu
`kullanicilar` tablosuna `ui_tercihleri JSONB DEFAULT '{}'` eklendi.
İleride tema, dil, bildirim ayarları da buraya eklenebilir.

### Geri Bildirim Fotoğraf Upload
Storage bucket `arespipe-dosyalar` için `feedback/` prefix INSERT policy eklendi.
`ares-layout.js`'de upload başarısız olursa base64 data URL olarak DB'ye kaydediliyor (fallback).

### Default Tema
`ares-layout.js` ve flash prevention scriptleri: `dark` → `light-anthracite`

### Topbar Canlı Saat
`ares-layout.js` topbar'ında her sayfada tarih+saat görünür, saniyede güncellenir.
`index.html`'deki ayrı saat kaldırıldı.

### Admin Panel — Yapılanlar Sekmesi
`admin/panel.html`'e ✅ Yapılanlar sekmesi eklendi. Geliştirme oturumu özetini gösterir.
Yol Haritası sekmesi sadece `[ ]` maddelerini gösterir — tamamlananlar artık Yapılanlar sekmesinde zaman damgasıyla görünür.
